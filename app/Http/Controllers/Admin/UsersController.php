    ...
    public function show($id)
    {
        ...
        $roles = \App\Role::get()->pluck('title', 'id')->prepend(trans('quickadmin.qa_please_select'), '');$user_actions = \App\UserAction::where('user_id', $id)->get();$internal_notifications = \App\InternalNotification::whereHas('users',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();$incomes = \App\Income::where('branch_id', $id)->get();$expenses = \App\Expense::where('branch_id', $id)->get();

        $user = User::findOrFail($id);

        return view('admin.users.show', compact('user', 'user_actions', 'internal_notifications', 'incomes', 'expenses'));
    }
